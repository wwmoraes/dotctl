...
def test_group_install(
  ...
  DOT_PATH = os.environ.get(dot_path_var, "")
  mocker.patch.object(sys, "argv", ["", group, "install"])
  bin_mock.set_echo("stow")

  from dotctl.cmd.dotctl import main
  bin_mock.activate()
  main(standalone_mode=False)
  bin_mock.deactivate()

  packages = [
    entry for entry in os.listdir(DOT_PATH) if not entry.startswith(".") and
    os.path.isdir(os.path.join(DOT_PATH, entry))
  ]

  for package in packages:
    script_output.add_output(
      messages.success(
        f"stow {Fore.LIGHTBLUE_EX}global{Style.RESET_ALL} {messages.package(package)}"
      )
    )
    script_output.add_output(
      "stow -d %s -t %s -R %s" % (
